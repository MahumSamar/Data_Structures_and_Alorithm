...
public:
    ...
    void InsertStudent(unsigned short int studentId, string name) {
        this->name = name;
...
public:
    ...
    void Search(int value) {
        ...
        loc = root;
        preloc = NULL;
        while (loc != NULL && loc->student.studentID != value) {
            // Until value is not found or reached the end of the tree
            if (value < loc->student.studentID) {
                preloc = loc;
                loc = loc->leftChild;
            } else {
                preloc = loc;
    ...
    void InsertWithoutDuplication(StudentData *value) {
        ...
        if (loc == NULL) {
            ...
            if (preloc != NULL) {
                ...
                if (value->studentID < preloc->student.studentID) {
                    //if value is less
    ...
    void PreOrder(AvlNode<Type> *treeNode) {
        ...
        if (treeNode != NULL) {
            cout << treeNode->student.studentID << "\t";
            cout << treeNode->student.name << "\n";
...
//            cout << "Node for which function called have value:\t" << treeNode->student.studentID << endl;
...
//            cout << "Node : " << treeNode->student << " at depth : " << d << endl;   //depth of each node printed
...
//        method to delete the value from the subtree
        Search(value->studentID);          //search the value in the BST
        if (loc == NULL) {
...
//        cout << "***********************************************************\n";
//        cout << "Node for which function called have value:\t" << x->student.studentID << endl;
...
//so that its address can be updated in its parent after rotation function returns.
            Search(treeNode->student.studentID);
...
public:
    ...
    void FileRead(AvlTree<StudentData> *avlTree) {
//        cout<<"File read method\n\n";
...
//        cout << endl;
        ...
        while (getline(file, line)) {
            ...
            //to split read row into 2 columns
            for (int i = 0; i <= 1; i++) {
                getline(ss, array[i], ',');
            }
            ...
            try {
                ...
                studentData->studentID=(unsigned short int)stoi(array[0]);
                studentData->name=array[1];
//                cout<<"array[0]"<<array[0];
//                cout<<"array[1]"<<array[1];
                avlTree->InsertWithoutDuplication(studentData);
            } catch (exception) {
...
int main() {
//to store the student name and roll number in avl tree
//
    Controller *controller = new Controller();

    AvlTree<StudentData> *avlTree = new AvlTree<StudentData>();

//    cout<<"file read function called\n";
    controller->FileRead(avlTree);
    cout<<"*********************************************\n";

    cout<<"Transversing AVL Tree in post order:\n";
    avlTree->PostOrder(avlTree->root);

    cout<<"*********************************************\n";
    cout<<"Transversing AVL Tree in pre order:\n";
    avlTree->PreOrder(avlTree->root);

    cout<<"*********************************************\n";
    cout<<"Transversing AVL Tree in In order:\n";
